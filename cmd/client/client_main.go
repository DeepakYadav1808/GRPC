...
func main() {
	...
	case "1\n":
		...
		err := bookclient.CreateBook(c)
		handleError(err)
	case "2\n":
		fmt.Println("Get Book entry")
		err := bookclient.GetBook(c)
		handleError(err)
	case "3\n":
		fmt.Println("Delete Book entry")
		err := bookclient.DeleteBook(c)
		handleError(err)
	case "4\n":
		fmt.Println("Update book entry")
		err := bookclient.Upatebook(c)
		handleError(err)

	case "5\n":
		fmt.Println("Server side streaming")
		err := bookclient.GetContent(c)
		handleError(err)
	case "6\n":
		fmt.Println("Getting requested book details")
		err := bookclient.GetBookdetails(c)
		handleError(err)
	default:
