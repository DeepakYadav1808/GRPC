...
func DeleteBook(bookId string) error {
	...
	err := Connectdb().Where(model.TABLE_PAGE_BOOKID+"=?", bookId).Delete(&model.Books{})
	if err.Error != nil {
		return err.Error
	}
...
func Updatebook(updateBookReq *bookstore_pb.UpdateBookRequest) error {
	...
	err := Connectdb().Model(&model.Books{}).Updates(newCol)
	if err.Error != nil {
		return err.Error
	}
	return nil
...
func RetrieveData(column_name string, column_value string) (*bookstore_pb.Streamresponse, error) {
	...
	err := Connectdb().Model(&model.Books{}).Select("*").Where(column_name+"= ?", column_value).Find(&book)
	if err.Error != nil {
